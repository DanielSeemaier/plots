...
create_performance_profile <- function(...,
    ...
    # Replace zero-cuts by 1, sort by primary key
    ...
    for (segment in segments) {
        ...
        pp_data <- pp_data %>% 
            dplyr::mutate(
                Transformed = ifelse(Transformed == 0 & Ratio >= from & Ratio < segment$to, 1, Transformed)
            ) %>%
            dplyr::mutate(
                Ratio = ifelse(
                    Transformed == 1, 
            ...
            dplyr::mutate(
                Transformed = ifelse(Transformed == 1, 2, Transformed)
            )
